...
uint64_t accesses_l3 = 0;
uint64_t read_misses_l3 = 0;
uint64_t write_misses_l3 = 0;
uint64_t write_back_l3 = 0;
...
tag_store L3_tag_store;
// End of defining global variables//


void setup_cache(uint64_t c1, uint64_t b1, uint64_t s1, uint64_t v, uint64_t c2, uint64_t b2, uint64_t s2, uint64_t c3, uint64_t b3, uint64_t s3){
    ...
    temp = pow(2, (c3 - b3 - s3)) * pow(2, s3);
    L3_tag_store.tag = new uint64_t[temp];
    L3_tag_store.index = new uint64_t[temp];
    L3_tag_store.lru_counter = new uint64_t[temp];
    L3_tag_store.dirty_bit = new bool[temp];
    L3_tag_store.valid_bit = new bool[temp];
...
//		 uint64_t temp_ind = j+(i*(2^(c2-b2-s2)));
    ...
    for (i = 0; i < temp; i++) {
        L3_tag_store.tag[i] = 0;
        L3_tag_store.index[i] = (i % (1ULL << s3));
        L3_tag_store.lru_counter[i] = 0;
        L3_tag_store.dirty_bit[i] = false;
        L3_tag_store.valid_bit[i] = false;
    }
...
void cache_access(char type, uint64_t arg, cache_stats_t* p_stats) {
    ...
    if (V == 0){
        ...
        for (int j = 0; j < num_of_tag_generate(2); j++) {
            if (L2_tag_store.tag[j] == tag_generator(arg, 2)) {
                if (L2_tag_store.index[j] == index_generator(arg, 2) &&
                    L2_tag_store.valid_bit[j] == true) {
                    hit_flag = 1; // L2 hit 발생
                    break;
                }
            }
        }

        if (hit_flag == 0) { // L2 miss
            access_sequence[2] = 'M'; // L2 miss 기록
            access_sequence[3] = '2';
            block_retrived_flag = L2_read_miss_fn(arg); // L2 miss 핸들링 함수 호출
            L2_read_miss++; // 통계 업데이트
            uint64_t arg_tag = tag_generator(arg, 3);
            uint64_t arg_index = index_generator(arg, 3);
            bool L3_hit = false;

            for (i = 0; i < num_of_tag_generate(3); i++) {
                if (L3_tag_store.tag[i] == arg_tag && L3_tag_store.index[i] == arg_index) {
                    if (L3_tag_store.valid_bit[i]) {
                        L3_hit = true;
                        L3_tag_store.lru_counter[i] = accesses;
                        break;
                    }
                }
            }
            ...
            if (!L3_hit) {
                read_misses_l3++; // Increment miss counter
                // Fetch block from memory or lower-level cache
            }
        }
        access_sequence[4] = ' ';
        access_sequence[5] = ' ';
    ...
    p_stats->avg_access_time_l2 = 0;
    p_stats->avg_access_time_l3 = 0;
...
int L3_read_hit_fn(uint64_t arg) {
    uint64_t j;
    uint64_t arg_tag = tag_generator(arg, 3);
    uint64_t arg_index = index_generator(arg, 3);

    // 태그와 인덱스에 해당하는 데이터를 찾고 LRU 카운터 업데이트
    for (j = 0; j < num_of_tag_generate(3); j++) {
        if (L3_tag_store.tag[j] == arg_tag &&
            L3_tag_store.index[j] == arg_index &&
            L3_tag_store.valid_bit[j]) {
            L3_tag_store.lru_counter[j] = accesses; // LRU 카운터 갱신
            return 1; // Hit 성공
        }
    }
    return 0; // Hit 실패 (이론상 발생하지 않음)
}

int L3_read_miss_fn(uint64_t arg) {
    uint64_t j;
    uint64_t j_low; // LRU 블록의 위치
    uint64_t temp_LRU_storage; // LRU 블록을 임시로 저장
    bool temp_LRU_dirty;
    uint64_t arg_tag = tag_generator(arg, 3);
    uint64_t arg_index = index_generator(arg, 3);
    int empty_flag = 0;
    ...
    // 빈 공간 확인
    for (j = 0; j < num_of_tag_generate(3); j++) {
        if (L3_tag_store.index[j] == arg_index && !L3_tag_store.valid_bit[j]) {
            empty_flag = 1;
            break;
        }
    }

    if (empty_flag == 0) { // 빈 공간이 없으면 LRU 블록 제거
        j_low = lru_block_finder(arg, 3);
        temp_LRU_storage = address_generate(L3_tag_store.tag[j_low], L3_tag_store.index[j_low], 3);
        temp_LRU_dirty = L3_tag_store.dirty_bit[j_low];

        // LRU 블록 초기화
        L3_tag_store.tag[j_low] = 0;
        L3_tag_store.valid_bit[j_low] = false;
        L3_tag_store.dirty_bit[j_low] = false;
        L3_tag_store.lru_counter[j_low] = 0;

        // LRU 블록이 dirty 상태라면 메인 메모리로 쓰기
        if (temp_LRU_dirty) {
        }
    }
    ...
    for (j = 0; j < num_of_tag_generate(3); j++) {
        if (L3_tag_store.index[j] == arg_index && !L3_tag_store.valid_bit[j]) {
            L3_tag_store.tag[j] = arg_tag;
            L3_tag_store.index[j] = arg_index;
            L3_tag_store.valid_bit[j] = true;
            L3_tag_store.dirty_bit[j] = false;
            L3_tag_store.lru_counter[j] = accesses;
            break;
        }
    }
...
int L3_write_hit_fn(uint64_t arg, bool dirty_bit) {
    uint64_t j;
    uint64_t arg_tag = tag_generator(arg, 3);
    uint64_t arg_index = index_generator(arg, 3);
    ...
    for (j = 0; j < num_of_tag_generate(3); j++) {
        if (L3_tag_store.tag[j] == arg_tag &&
            L3_tag_store.index[j] == arg_index &&
            L3_tag_store.valid_bit[j]) {
            L3_tag_store.dirty_bit[j] = dirty_bit; // Dirty bit 업데이트
            L3_tag_store.lru_counter[j] = accesses; // LRU 카운터 갱신
            return 1; // Hit 성공
        }
    }
...
int L3_write_miss_fn(uint64_t arg, bool dirty_bit) {
    uint64_t j;
    uint64_t j_low;
    uint64_t temp_LRU_storage;
    bool temp_LRU_dirty;
    ...
    for (j = 0; j < num_of_tag_generate(3); j++) {
        if (L3_tag_store.index[j] == arg_index && !L3_tag_store.valid_bit[j]) {
            ...
            L3_tag_store.index[j] = arg_index;
            L3_tag_store.valid_bit[j] = true;
            L3_tag_store.dirty_bit[j] = dirty_bit; // Dirty bit 설정
            L3_tag_store.lru_counter[j] = accesses;
            break;
        }
    }
...
void complete_cache(cache_stats_t *p_stats) {
    ...
    double HT3;
    double MR3;
    double MP3 = 0.0000005;//*(pow(10,-6)); Devide by 1e-6 later
    double MRVC;
    double AAT3;
    ...
    HT2 = 4 + ((0.4)*(L2_size[2]));
    HT3= 8 + ((0.6)*(VC_size[2]));
    MR3 = victim_misses/(p_stats->accesses_vc);
    cout << HT2 << '\n';
    ...
    p_stats->avg_access_time_l2 = AAT2;
    p_stats->avg_access_time_l3 = AAT3;

    cout << p_stats->avg_access_time_l1 << '\n';
    cout << p_stats->avg_access_time_l2 << '\n';
    cout << p_stats->avg_access_time_l3 << '\n';
}
